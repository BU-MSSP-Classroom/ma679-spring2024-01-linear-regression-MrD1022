...
```{r,eval=FALSE,echo=show_code}
log_pred_2009 <- predict(lmfit_2008, newdata=ames_raw_2009)
pred_2009 <- na.omit(exp(log_pred_2009))
...
Your code:
```{r,echo=TRUE}
ames_raw_2009$pred <- exp(predict(lmfit_2008, newdata=ames_raw_2009))

melt_2009_actual <- melt(ames_raw_2009[, c("Lot Area", "Year Built", "Year Remod/Add",
                                       "Total Bsmt SF", "Full Bath", 
                                       "Gr Liv Area", "1st Flr SF",
                                       "Garage Area", "SalePrice")],
                  id.vars = "SalePrice")
melt_2009_pred <- melt(ames_raw_2009[, c("Lot Area", "Year Built", "Year Remod/Add",
                                       "Total Bsmt SF", "Full Bath", 
                                       "Gr Liv Area", "1st Flr SF",
                                       "Garage Area", "pred")],
                  id.vars = "pred")
combinded_2009 <- merge(melt_2009_actual, melt_2009_pred, 
                        by = c("variable", "value"), all.x = TRUE, sort = FALSE)
ggplot(combinded_2009) + geom_point() +
  aes(x = variable, y = SalePrice) +
  #geom_smooth(method = "lm", se = FALSE) +
...
~~~
Horsepower is a significant predictor.
...
There is a negative relationship since coefficient for horsepower is less than 0, mpg will decrease as horsepower grows.

ii. How strong is the relationship between the predictor and the response?

p-value of horsepower is small enough thus this predictor is statistically significant; 
R-squared is around 60%, meaning there are other relationships not captured using this model.
