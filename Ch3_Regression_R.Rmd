...
ggplot(ames_raw) + geom_histogram(fill = "skyblue") + # SalePrice histogram
  aes(x = SalePrice) + 
  geom_vline(xintercept = mean(ames_raw$SalePrice), col = "red") # mean of SalePrice
summary(ames_raw$SalePrice)
```

Initial linear model without a predictor
```{r,echo=show_code}
# Modeling: null model for SalePrice
...
# Lower RMSE means better fit, but here it grows. 
```
...
```{r,echo=show_code,fig.width=7,fig.height=4.5}
# Visualization: Uncertainty (Prediction Interval)
intpred = predict(lmfit_null, interval = "prediction") # to obtain prediction interval
ggplot(ames_raw) + 
  geom_histogram(fill = "skyblue") + 
  aes(x = SalePrice) + 
  geom_vline(xintercept = mean(ames_raw$SalePrice),
             col="red") + 
...
ggplot(ames_raw) + 
  geom_histogram(fill = "skyblue") + aes(x = SalePrice) + 
  geom_vline(xintercept = exp(mean(log(ames_raw$SalePrice))), 
             # Calculate mean on log scale then convert
             col="red") + scale_x_log10()+
...
```{r,echo=show_code}
allres <- residuals(lmfit_null) # actual - predict
# allres < 0: overpayment; allres > 0: missed profit
abs( sum(allres[allres<0])) + sum(0.1*(coef(lmfit_null) + allres[allres>0]) ) 
# coef(lmfit_null) = 180796.1
# coef(lmfit_null) + allres[allres>0] = House Price
```

```{r,echo=show_code}
# Loss function on log transformation
allreslog <- ames_raw$SalePrice - exp(predict(lmfit_null_log)) # residuals
abs( sum(allreslog[allreslog<0])) + 
  sum(0.1*(exp(coef(lmfit_null_log)) + 
             allreslog[allreslog>0]))
```
...
lmfit_liv_area_1 <- lm(log(SalePrice) ~ `Gr Liv Area`, ames_raw) # log trans
lmfit_liv_area_2 <- lm(log(SalePrice) ~ log(`Gr Liv Area`),ames_raw) # log-log
...
```{r,fig.width=9,fig.height=7,echo=show_code}
mames <- melt(ames_raw[,c("Lot Area","Year Built", "Year Remod/Add",
                          "Total Bsmt SF", "1st Flr SF",
                          "2nd Flr SF", "Low Qual Fin SF",
                          "Gr Liv Area", "Full Bath",
                          "Half Bath", "Bsmt Full Bath",
                          "Bsmt Half Bath", "TotRms AbvGrd",
                          "Garage Area", "Wood Deck SF",
                          "Open Porch SF", "Pool Area","SalePrice")],
              id.vars = "SalePrice")
ggplot(mames) + geom_point() + 
...
```{r,fig.width=6,fig.height=4,echo=show_code}
ggplot(ames_raw) + geom_point() + 
  aes(x = `Lot Area`,y = SalePrice) + 
  xlab("Lot size in square feet") + 
  ylab("Sale Price") + 
  geom_smooth(method = "lm", se = FALSE) + 
  scale_y_log10() + scale_x_log10() #+facet_grid(~`Bedroom AbvGr`)
```
...
ggplot(ames_raw) + geom_point(alpha=0.3) + 
  ...
  theme(legend.position = "none") + facet_wrap(~Neighborhood)
...
lmfit_2008<- lm(log(SalePrice) ~ `Year Built` + `Year Remod/Add` +
                  ...
                  `Total Bsmt SF` + `1st Flr SF` + 
                  `Garage Area`, 
                ames_raw_2008) # use ames_raw_2008
summary(lmfit_2008)
```
Predictors chosen according to correlation map above: 
predictors having correlation over 0.5 with SalePrice.


When you decide on your model use the following to come up with your test loss.
```{r,eval=FALSE,echo=show_code}
pred_2009 <- na.omit(exp(predict(lmfit_2008, newdata=ames_raw_2009)))
...
Your code:
```{r,echo=TRUE}
# It seems R code chunks won't run in a linear fashion. so I copied previous def. here
lmfit_2008<- lm(log(SalePrice) ~ `Year Built` + `Year Remod/Add` +
                  `Full Bath` + `Gr Liv Area` +
                  `Total Bsmt SF` + `1st Flr SF` + 
                  `Garage Area`, 
...
~~~
This model has a shrinking RMSE of 68504.59, indicating a better fit than null model or single-predictor models.
~~~
...
~~~
As seen in the summary, we have large standard errors in "Full Bath" and "1st Flr SF", so these two variables could be excluded.
~~~
...
lmfit_2008_2 <- lm(formula = log(SalePrice) ~ 
                     `Year Built` * `Year Remod/Add` + 
                     `Gr Liv Area` * `Total Bsmt SF` + `Garage Area`, data = ames_raw_2008)
summary(lmfit_2008_2)
ames_raw_2009$pred <- exp(predict(lmfit_2008_2, ames_raw_2009))
ames_raw_2009$difpred <- ames_raw_2009$SalePrice-ames_raw_2009$pred
plot(ames_raw_2009$pred, ames_raw_2009$difpred, 
     main = "Residuals in 2nd Model", cex = 1.5, pch = 20,
     ylab = "Residuals", xlab = "Predicted Value")
abline(h = 0)
```
...
~~~
Here I drop two not significant variables and add interaction between Gr Liv Area" and "Total Bsmt SF". The interaction is significant, we may think it makes sense. Interaction between "Year Built" and "Year Remod/Add" is considered based on correlation map where the two show strong correlation. However, their interaction does not make very much sense in the summary.
...
Your code:
```{r,echo=TRUE}
lmfit_2008_poly_1 <- lm(formula = log(SalePrice) ~ 
                     `Year Built` + I(`Year Built`^2) + I(`Year Built`^3), 
                     data = ames_raw_2008)
lmfit_2008_poly_2 <- lm(formula = log(SalePrice) ~ 
                     `Year Remod/Add` + I(`Year Remod/Add`^2) + 
                       I(`Year Remod/Add`^3), data = ames_raw_2008)
lmfit_2008_poly_3 <- lm(formula = log(SalePrice) ~ 
                     `Gr Liv Area` + I(`Gr Liv Area`^2) + 
                       I(`Gr Liv Area`^3), data = ames_raw_2008)
lmfit_2008_poly_4 <- lm(formula = log(SalePrice) ~ 
                     `Total Bsmt SF` + I(`Total Bsmt SF`^2) + 
                       I(`Total Bsmt SF`^3), data = ames_raw_2008)
lmfit_2008_poly_5 <- lm(formula = log(SalePrice) ~ 
                     `Garage Area` + I(`Garage Area`^2) + 
                       I(`Garage Area`^3), data = ames_raw_2008)

summary(lmfit_2008_poly_1)
summary(lmfit_2008_poly_2)
summary(lmfit_2008_poly_3)
summary(lmfit_2008_poly_4)
summary(lmfit_2008_poly_5)
```

Your answer:

~~~
According to p-value shown above, variables "Total Bsmt SF" and "Garage Area" could bear non-linear association with response variable.
~~~
...
Your code:
```{r,echo=TRUE, fig.width=8, fig.height=6}
lmfit_2008_3 <- lm(formula = log(SalePrice) ~ 
                     `Year Built` + `Year Remod/Add` + 
                     `Gr Liv Area` * `Total Bsmt SF` + `Garage Area` + 
                     I(`Total Bsmt SF`^2) + I(`Total Bsmt SF`^3), 
                   data = ames_raw_2008)
ames_raw_2009$pred <- exp(predict(lmfit_2008_3, ames_raw_2009))
ames_raw_2009$difpred <- ames_raw_2009$SalePrice-ames_raw_2009$pred
plot(ames_raw_2009$pred, ames_raw_2009$difpred, 
     size = 4, main = "Residuals in 3rd Model", 
     xlab = "Predicted Value", ylab = "Residuals")
abline(h = 0)
# ChatGPT
d <- ames_raw_2009[rank(ames_raw_2009$difpred, ties.method = "first") <= 5, ]
d
```
...
~~~
Their PID are 0534427010, 0923276250, 0528108020, 0533350090, 0908154080, the five houses having lowest actual-prediction value. 
As seen from the residual plot, the residual is balanced when predicted sale price is below $25,000. However the residuals grow larger as the house price increase. Thus, this model is not capable for relatively expensive houses.
~~~
...
~~~
Their PID are 0528150070, 0528106020, 0528110020, 0528110090, 0528164060.
~~~
...
~~~
Horsepower is a significant predictor for mpg.
~~~
...
i. Is there a relationship between the predictor and the response?
 ...
 - Yes, p-value is really small. There is a negative relationship since coefficient for horsepower is less than 0, mpg will decrease as horsepower grows.

ii. How strong is the relationship between the predictor and the response?

 - p-value of horsepower is small enough thus this predictor is statistically significant; R-squared is around 60%, meaning the model can explain 60% of mpg value, and there are other relationships not captured using this single-predictor model.
...
```{r}
hrs_98 <- data.frame(horsepower = 98)
predict(lm_8.a, data.frame(horsepower = 98))
# ChatGPT
intvl_confi_8.a <- predict(lm_8.a, hrs_98, interval = "confidence", level = 0.95)
intvl_confi_8.a
intvl_pred_8.a <- predict(lm_8.a, hrs_98, interval = "prediction", level = 0.95)
intvl_pred_8.a
```
...
ggplot(auto) + aes(x = horsepower, y = mpg) +
  geom_point() + 
  geom_abline(slope = coef_8[2], intercept = coef_8[1], color = "blue") +
  ggtitle("8.b Linear Regression between mpg and horsepower")
```
...
```{r,echo=TRUE, fig.width=8, fig.height=6}
# Residual Plot
resid_8.c <- residuals(lm_8.a)
plot(predict(lm_8.a, auto), resid_8.c, 
     xlab = "Predicted Value", ylab = "Residuals", 
     main = "Residuals on Linear Model 8.a")
abline(h = 0)
```
...
~~~
The residual plot is unbalanced when predicting mpg value under 20. The residual data tend to cluster to the right hand side. 
~~~
...
Your code:
```{r,echo=TRUE, fig.width=8, fig.height=6}
mames_9 <- melt(auto[,c("cylinders", "displacement",
                      "horsepower", "weight",
                      "acceleration", "year",
                      "origin", "name","mpg")],
              id.vars = "mpg")
ggplot(mames_9) + geom_point() + 
  aes(x=value,y = mpg) + 
  facet_wrap(variable~.,scales = "free")
```
...
M <-cor(auto[,c("cylinders", "displacement", "horsepower", "weight",
...
~~~
Horsepower, cylinders, weight, displacement have strong negative correlation to mpg, while the other three quantitative variables have weak positive effects. 
Plus, for description I think is quantitative not qualitative variables to be mapped in corr() function. 
~~~
...
Your code:
```{r,echo=TRUE}
lm_9.c <- lm(mpg ~ cylinders + displacement + horsepower + weight + 
...
~~~
Adjusted R-squared over 0.81 suggests the model fits well yet still cannot describe some part of the data set.
~~~
...
i. Is there a relationship between the predictors and the response?
 ...
 - `*Yes, p-value for displacement, weight, year and origin is significant.*`
...
iii. What does the coefficient for the year variable suggest?
 ...
 - Year has a positive effect on mpg, their values change in the same direction. The more to-date the vehicles are, the stronger mpg will be.
...
```{r,echo=TRUE, fig.width=8, fig.height=6}
#Residuals
plot(predict(lm_9.c, auto), resid(lm_9.c), 
     xlab = "Predicted Value", ylab = "Residuals",
     main = "Residual Plot in Problem 9.c")
abline(h=0)
#Leverages, ChatGPT
plot(hatvalues(lm_9.c), 
     ylab = "Leverage Values", xlab = "Observation Number",
     main = "Leverage Plot in Problem 9.c")
abline(h = 2 * mean(hatvalues(lm_9.c)), col = "red")
```

Your answer:

~~~
Outliers when predicting over 30.
There are several observations over the mean leverage line, suggesting there could be values that may affect statistics.
...
Your code:
```{r,echo=TRUE}
lm_9.e <- lm(mpg ~ cylinders:horsepower + displacement:origin + weight + 
...
~~~
I don't know about cars, I guess and read correlation plot.
cylinders:horsepower, displacement:origin, horsepower:acceleration are all statistically significant.
~~~


(f) Try a few different transformations of the variables, such as $log(X)$, $\sqrt{X}$, $X^2$. Comment on your findings.
```{r}
lm_9.f <- lm(mpg ~ I(cylinders^2) + cylinders:horsepower + 
               sqrt(horsepower) + log(acceleration) +
               displacement:origin + weight + acceleration * horsepower + 
               year + origin, auto)
summary(lm_9.f)
```
...
attach(Carseats)
carseats <- Carseats
lm_10.a <- lm(Sales ~ Price + Urban + US, data = carseats)
summary(lm_10.a)
```
...
~~~
The Adj. R-square is only 0.23, the model is not persuasive.
~~~
...
~~~
Intercept: the expected Sales value is 13.04 when Price = 0, answers of Urban and US are both No. 
Price: a significant statistics, Sales decreases when Price rises.
US: keep other variables unchanged, if US is yes, Sales will go up.
Urban: not a significant predictor.
~~~
...
Your code:
```{r,echo=TRUE}
lm_10.e <- lm(Sales ~ Price + US, carseats)
summary(lm_10.e)
```
...
~~~
Urban is not significant thus dropped.
~~~
...
~~~
Looking into adj. R-squared, the latter model is slightly improved, yet the poor number suggests a vast need of improvement.
~~~
...
#Residuals
plot(predict(lm_10.e, carseats), resid(lm_10.e), 
     xlab = "Predicted Value", ylab = "Residuals",
     main = "Residual Plot in Problem 10.e")
abline(h=0)
#Leverages
plot(hatvalues(lm_10.e), 
     ylab = "Leverage Values", xlab = "Observation Number",
     main = "Leverage Plot in Problem 10.e")
abline(h = 2 * mean(hatvalues(lm_10.e)), col = "red")
```
...
~~~
As shown in residual plot and leverage plot, there are no obvious outliers and high leverage observations.
~~~
